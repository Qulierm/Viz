...
**The popover is repainted in the native Wi-Fi panel's language (supersedes the panel radius, the panel edge and the tile fills above).** The reference is the system's own Wi-Fi popover (Control Center style), measured from a screenshot of it in this session at **2x**, so the image pixels are halved for points. Every value below is mapped to a **semantic system colour or shape**, never to the sampled triplet, so appearance, accent colour and increased-contrast settings keep working; the sampled numbers are acceptance evidence.
...
| panel corner radius | 14 px = ~7 pt | `StatusItemController.panelCornerRadius` = 7 pt |
| internal dividers | 2 px = 1 pt, 73,75,77 | `NSColor.separatorColor` (white at ~11 % in dark appearance; measured 71,72,75 over the panel tone) |
| section label text | 133,135,137 | `NSColor.secondaryLabelColor` |
| row label text | 243,249,255 | `NSColor.labelColor` |
| hovered/selected row | 71,76,86 | `NSColor.unemphasizedSelectedContentBackgroundColor` (measured 70,70,70 in dark, 220,220,220 in light) |
| hovered row corner radius | ~14 px = ~5-7 pt | `VizTheme.cornerControl` = 6 pt, continuous |
| icon badge | 54 px = 27 pt circle (grey badge 52 px) | `VizTheme.badgeDiameter` = 27 pt |
| active badge fill, white glyph | 0,147,255 | `NSColor.controlAccentColor` (0,122,255 on this machine) |
| inactive badge fill, lighter grey glyph | 65,66,70 | `NSColor.quaternaryLabelColor` (white at 10 %; 71,72,75 over the panel tone - `secondarySystemFill` would be 67,68,71 but is macOS 14+ and this app deploys to macOS 13) |
| outer border | none | none: separation comes from the material and the window shadow |

What changed: the panel's corner went from 16 pt to **7 pt** and its **0.5 pt outer border is gone** with its constant (the reference draws no outer edge; the only hairlines in it are internal dividers). The tiles are now the reference's rows: **transparent at rest** - the panel's material shows through - and filled with the system's unemphasized selection colour while hovered, in a continuous 6 pt corner. Each tile's glyph sits in a **27 pt circular badge**: `controlAccentColor` on the popover's primary action (Capture, marked `primary: true` in `MainView`) and `quaternaryLabelColor` on the other four. The glyph is white on the accent badge, as the reference draws the connected item, and follows the label colour on the neutral ones - the reference's own "lighter grey glyph" in dark appearance (225 over the neutral fill, ~8.8:1 contrast) and a dark glyph in light appearance (~10.8:1) where a fixed white glyph on the light neutral fill would measure ~1.3:1 and vanish. The `vizGlassControl` helper and its interactive glass are gone with the fills; the badge is drawn as a background of the existing 18 pt icon slot, so the per-symbol point-size table and the 24x18 slot still normalise the five glyphs optically and the popover keeps its **480x99 pt** size instead of growing for the badge.

Deliberate deviations from the reference, all measured: the **hint capsule stays** with its white text (an earlier explicit instruction; the reference has no key-hint chips, and `hintbright` is unchanged at 38043 light / 549 dark); the row **stays one row of five** rather than becoming the reference's row list, because the compact popover was requested earlier - the five-row list remains available as a follow-up; and the neutral badge's glyph is the label colour rather than a fixed white, for the light-appearance contrast above.

In the design-check section the ported values are asserted in three places. `statuspanel` reads the panel's chrome off the live panel: `native=cornerRadius:ok(7.0 == StatusItemController.panelCornerRadius (7.0), documented 7.0 +/- 0.5)` - compared both with the app's own constant and with the documented reference value, so neither a constant change nor a hard-coded layer radius can drift - `native=borderWidth:ok(0.0 == 0 ...)` (the edge must stay absent; a 0.5 pt border fails), and the unchanged curve, material, blending, level, opacity, deactivation, appearance-duration and Reduce-Motion entries. A new **`nativebadges`** check guards the badges: `badgeDiameter:ok(27.0 == documented 27.0 +/- 0.5)`, `tileRadiusRange:ok(6.0pt inside the reference's 5.0-7.0pt range)`, `badgeDiameterRendered:ok(accent=54px neutral=54px light=54px (expected 54 +/- 2))` from a capture of the real tile, `badgeFills:ok(accent 0,122,255 == controlAccentColor 0,122,255, neutral 55,55,59 == quaternaryLabelColor over the backdrop 54,54,59)` and `badgeGlyph:ok(accent 0.59 brighter, neutral 0.66 brighter (dark), neutral-light 0.71 darker)`. The tile's own fills and corner stay in **`nativetiles`**: `tileRestFill:ok(inside the tile 33,33,38 == backdrop 33,33,38)` (nothing at rest), `tileHoverFill:ok(hovered 70,70,70 == unemphasizedSelectedContentBackgroundColor 70,70,70)` (the hovered fill is the system's colour), `tileCornerRendered:ok(depth=1.0pt (ceiling 2.0) inset=2.5pt (ceiling 4.0))` - calibrated against measurements: the 6 pt reference corner reads 1.0/2.5 pt, the 11 pt corner this replaced 4.0/6.5 and the 16 pt chamfer 7.5/11.0. Every entry was proven to discriminate with a temporary edit: a 16 pt panel radius fails `cornerRadius`, a 0.5 pt border fails `borderWidth`, an always-on resting fill fails `tileRestFill` (and `alignment`, because a filled tile merges the ink runs the badge is found from), and a 20 pt badge fails `badgeDiameter` and `badgeDiameterRendered`.

The port moved the measured numbers, and each shift is explained. `icons` ink went 108 -> **1426** at all four heights because the band now holds the badge and its white glyph rather than the glyph's ink alone, and the band moved one device pixel (46-75 -> 47-76) because the label's bright run lost its top antialiased row once the tile's fill stopped lightening it behind the label; the 60-pixel threshold is unchanged. `popoverheight` is unchanged at **480x99.0** (ceiling 109) - the badge grows into the tile's padding by construction. `alignment` was rewritten for the badge design (the old metric measured a bright-ink icon box, which the accented badge would have inflated for one button only) and every limit is unchanged: `badgeWidthSpread` 0 px (54/54/54/54/54, limit 4), `glyphWidthSpread` 2 px (31/29/29/30/30, limit 4 - the optical-normalisation guard, now measured at the 0.75 brightness that isolates a white glyph from either badge fill), `badgeCentres` 1.5 px (limit 3), `badgeRow` 0.0 px (limit 2), `labelRow` 0.0 px (limit 2), `pillRow` 1.5 px (limit 2), and `inkBalance` - which measured the icon block's centring inside the tile's fill, a fill that no longer exists - is replaced by `badgeGap` (7.5 pt in all five, worst 0.0 pt, limit 3). `design`'s popover `nonblank` fell 54.1 % -> **15.1 %** light and 54.6 % -> **15.4 %** dark because the tiles have no fill any more; `buttons` rose 0.00 % -> **1.42 %** with **neutralColumns=0.00 %** - the rule is now "exactly one accented badge, and the four neutral columns stay as neutral as the whole band used to be" (ceiling 2.4 %, documented against the measured 1.42 %); `accent` 0 -> 2181 (the accent badge); `hintbright` unchanged. `previewclose` (34 px / 308) is untouched. The suite is now **15 checks** and `bash scripts/render-check.sh` runs green twice with identical output.

**On-screen confirmation of the port (the harness cannot photograph this part).** Liquid Glass cannot be rasterised offscreen and a `behindWindow` material is resolved by the WindowServer - a probe rendering `material = .menu` over three different backdrops returned a flat 66,66,66 every time - so the panel's *tone* is the one reference value that has to be judged by eye. The app adds no colour of its own over the material (`popovercontent` still measures delta 0/0/0 in both appearances), so what shows on screen is the system material over the desktop, exactly as the reference panel does. By eye:

- open the popover over a wallpaper: the panel's corners should look much tighter than before (~7 pt) and there should be **no line along the panel's edge** - just the material and its shadow;
- the five icons should sit in equal 27 pt circles, with Capture's circle vivid accent blue and the other four neutral grey, in both light and dark appearance;
- pointing at a tile should fill the tile's rounded rectangle (6 pt) with the system's selection grey, with the badge and label on top - and there should be no tile edge at all until that happens;
- the popover should still merge with the menu bar rather than sitting on a slab of its own colour, and it should still close on a second click on the icon, on Esc and on a click away;
- with Reduce Motion on (System Settings -> Accessibility -> Display) it should appear instantly at full size, as before.


**The panel and its tiles carry the system's own chrome (supersedes the 12 pt panel corner and the hand-made tile cues above).** `StatusItemController` names the three chrome values: `panelCornerRadius` **16 pt**, `panelBorderWidth` **0.5 pt** and `panelAppearDuration` **0.12 s**. The effect view's layer now gets `cornerCurve = .continuous` (a `CALayer`'s default is the circular curve), that radius, and a half-point border whose colour is `NSColor.separatorColor` - the hairline the system's own panels draw instead of a coloured stroke. The appearance is a single 0.12 s `easeOut` fade from 98 % scale and zero opacity, so the panel arrives the way the system's panels do, and Reduce Motion (`NSWorkspace.shared.accessibilityDisplayShouldReduceMotion`, read through the controller's `shouldReduceMotion`) skips the animation entirely and shows the panel at full size at once. The dismissal stays immediate on purpose: an animated dismissal would leave `panel.isVisible` true while it runs, which would break the toggle, suppression and grace semantics the lifecycle steps assert. The tile lost the two cues that made the row look drawn rather than native: the `0.95` press scale (macOS dims or lightens a control while it is held down, it never scales it) and the 1 pt `strokeBorder` ring drawn over the tile (`isHovered ? primary 0.22 : secondary 0.18`). On macOS 26+ the tile's hover and press response is the glass itself - `glassEffect(.regular.interactive())` - while the material fallback (macOS 13-25, and the harness, which cannot rasterise glass offscreen) has no interactive glass and keeps one hairline that the hover state only brightens, **0.10 to 0.16**, with the hover animation moved from 0.3 s to the system's **0.15 s** (`RoundedRectangleButtonStyle.hoverDuration`). Corners are **continuous** on both the panel and the tiles, and the tile's corner is one shape: `VizTheme.controlShape(radius:)` = `RoundedRectangle(cornerRadius:style: .continuous)` at `VizTheme.cornerControl` = **11 pt** (it was 16), used by the glass, by the fallback's fill and edge and by the clip that trims the tile, so the two rendering paths cannot drift apart. The curve is stated explicitly rather than left to `RoundedRectangle`'s default, because that default is not the same across the range the app supports - the current SDK already defaults to continuous, older ones default to circular.

The harness asserts each of these by name. `statuspanel` now reads the chrome off the panel the app builds, after presenting it once more with `recordsPresentationOnly` set (the presentation is recorded and no window is ordered on screen), and compares every value with the app's own constant rather than a copy: `native=cornerCurve:ok(continuous)`, `native=cornerRadius:ok(16.0 == StatusItemController.panelCornerRadius (16.0))`, `native=borderWidth:ok(0.5 == StatusItemController.panelBorderWidth (0.5))`, `native=borderColour:ok(1.000,1.000,1.000 a0.10 == separator 1.000,1.000,1.000 a0.10)`, `native=material:ok(menu)`, `native=blending:ok(behindWindow)`, `native=level:ok(101 == popUpMenu)`, `native=opaque:ok(isOpaque=false)`, `native=hidesOnDeactivate:ok(hidesOnDeactivate=false)`, `native=appearDuration:ok(0.12s, inside the system's 0.05-0.20s band)` and `native=reduceMotion:ok(forcedOff=animates forcedOn=skips shownFullSize=true)`. The new `nativetiles` check does the same for the tiles: `native=tileCornerRadius:ok(shape 11.0 == VizTheme.cornerControl (11.0), documented 11.0 +/- 0.5, tile call sites=glass+clip)`, `native=tileCornerCurve:ok(continuous)`, `native=tileHoverDuration:ok(0.15s, inside the system's 0.10-0.20s band)`, `native=tileFallbackEdge:ok(rest=0.2165 (ceiling 0.26) hovered=0.2597 (must rise))`, `native=tileCornerRendered:ok(depth=2.0pt (ceiling 3.5) inset=6.5pt (ceiling 9.0))`, `native=tilePressScale:ok(1.0, the tile's body reads no press state)` and `native=tileRing:ok(tile=none shippingBranch=none fallbackStrokes=1)`. Every entry was shown to discriminate with a temporary edit that was never committed: a circular curve fails `tileCornerCurve` (`circular`), a 12 pt tile radius fails `tileCornerRadius`, the old 16 pt chamfer fails `tileCornerRendered` (`depth=5.0pt (ceiling 3.5) inset=11.0pt (ceiling 9.0)`), a restored 0.95 press scale fails `tilePressScale`, a restored ring fails `tileRing` (`tile=ring`), a hard-coded 12 pt panel radius fails the panel's `cornerRadius` entry (`12.0 == StatusItemController.panelCornerRadius (16.0)`) and dropping the Reduce Motion branch fails `reduceMotion` (`shownFullSize=false` - the decision accessor alone still read correctly, so it is the real presentation that catches a branch which ignores the setting).

The renders moved only where the corner changed. The popover's `nonblank` share rose from 52.9 % to **54.1 %** (light) and 53.4 % to **54.6 %** (dark) because a tighter corner fills slightly more of the tile's own box; the tiles are still neutral (`buttons=0.00 %` in both appearances) and the white hints are untouched (`hintbright` 38043 light / 549 dark, identical to before). `alignment` is unchanged in every size and position metric (spread 1 px, worst icon centre 1.3 px, icon row 1.2 px, label row 0.0 px, worst pill 1.5 px), but the two ink gaps it reports moved by 2.5 pt on both sides - above the icon `4.5/7.0/4.5/3.5/5.5` to `7.0/9.5/7.0/6.5/8.5` and below the label `3.0/5.0/5.0/2.5/4.5` to `5.5/7.5/7.5/5.5/7.5` - because those gaps are sampled 4 pt inside the tile's left edge, where the smaller corner reaches the tile's straight edge sooner; their difference, which is what `inkBalance` asserts, is unchanged at **2.0 pt** (limit 3). The suite is now **14 checks** and `bash scripts/render-check.sh` runs green twice with identical output.

**On-screen confirmation (the harness cannot photograph this part).** Offscreen captures cannot rasterise Liquid Glass, and a `behindWindow` material samples the desktop, so the panel's chrome and the glass appearance are asserted structurally and confirmed by eye:

- the panel's two top corners carry the system's continuous curve and a single half-point separator hairline sits on the edge, in both light and dark appearance;
- hovering a tile answers with the glass itself (a soft highlight) and holding it down dims or lightens it without the tile shrinking - no hard ring appears along the tile edge;
- opening the popover fades and scales it in over about a tenth of a second, and with Reduce Motion on (System Settings -> Accessibility -> Display) it appears instantly at its full size;
- the popover still merges with the menu bar rather than sitting on a panel-coloured slab, and it closes on a second click, on Esc and on a click away.

**The local model engine was removed - the OvisOCR2 section above is superseded.** The feature was built and evaluated end to end: the pinned llama.cpp runtime loaded the pinned GGUF weights, and the model parsed a generated test page into correct Markdown (heading, paragraph, an HTML table, a LaTeX formula) in roughly six seconds and about 4 GB of memory, fully offline. On real captures, however, its results were worse than Apple Vision's, so at the user's request the whole feature came out again: the recognition engine setting and the `LocalModelRecognizer` with its fallback, the `OvisModel` catalogue and `ModelStore` manager, the fifth Recognition settings tab and the preview's fallback note, the bundled llama.cpp runtime and the `bundle_runtime()` step that copied it, the three fetch/spike scripts, the five model-path checks and the opt-in real run, and the raised tab-strip expectation (back to `tabStripMinimumRuns = 3`, measured `58%/4runs`). The app is Vision-only again with its request configuration and dispatch exactly as they were before the model work, the bundle is back to about 5.7 MB with `Contents/Resources` holding only the icon and the sound, and the 763 MB of model files were removed from Application Support after their SHA-256 values were verified against the pinned ones. The history is unchanged - the model work stays in the branch's past and this removal is a later commit.

**Optional local recognition engine (OvisOCR2).** Apple Vision stays the default engine; the local model is an explicit opt-in for page-level parsing into Markdown with tables and formulas. Nothing about the model is ever committed or bundled - the weights live in the user's Application Support folder and the runtime is fetched at build time into `build/` and copied into the app bundle.

**Runtime.** `scripts/fetch-runtime.sh` pins llama.cpp release **b11160** (`llama-b11160-bin-macos-arm64.tar.gz`, SHA-256 `5679b3e952772a9f9a39f9d42d7f0eb3d4c424103fe56f5516507583a0c6e3fa`), verifies the asset before unpacking, and fetches the MIT licence text alongside it. `scripts/build-app.sh` calls `bundle_runtime()` **before signing** (the signature seals resources, so a later copy invalidates it) and puts the CLI, its dylibs and `LICENSE-llama.cpp` in `Contents/Resources/Runtime/` (~57 MB). If the fetch fails the build warns and the app runs with Vision only.

**Model.** `scripts/fetch-ovis-model.sh` pins `bartowski/ATH-MaaS_OvisOCR2-GGUF` at revision `ab22420f3d44201d3aa5a62ca49a665a46b507e9` (converted from `ATH-MaaS/OvisOCR2`, Apache-2.0): `ATH-MaaS_OvisOCR2-Q4_K_M.gguf` 557,867,136 bytes / `3786d230…`, `mmproj-ATH-MaaS_OvisOCR2-f16.gguf` 204,987,040 bytes / `4e0e9cb9…` - 753 MB in `~/Library/Application Support/Viz/Models/OvisOCR2`. The same values are pinned in `OvisModel.files` in `Viz/Logic/OCR.swift`, which the Recognition tab's `ModelStore` uses: downloads stream to a `.part` file, are checked for size and SHA-256 (CryptoKit) and only then moved into place; a mismatch deletes the file. `scripts/ovis-spike.sh` runs the whole thing headless on a generated test page.

**Command line and flags** (proven by the spike and used by `LocalModelRecognizer.arguments(imageURL:)`): `llama-mtmd-cli -m <model> --mmproj <projector> --image <page.png> -p "<model card prompt>" -n 4096 --jinja -t 8`, with the model card's prompt verbatim and a 120 s timeout. This build rejects `--chat-template-kwargs`, so the model card's `enable_thinking=False` cannot be set through the CLI; the answer always follows a line containing `</think>` and the recogniser strips everything up to it, plus the runtime's own log lines.

**Latency and memory** (Apple silicon, this machine): about 6-14 s and ~4.2 GB peak RSS per page, against milliseconds for Vision - which is why Vision is the default.

**Fallback.** `TextRecognition.localModelRecognizerIfReady` returns the local recogniser only when the setting is `localModel` **and** the bundled runtime resolves **and** both model files are present with their pinned sizes. Any thrown error from the model path (missing runtime, missing or corrupt model, timeout, non-zero exit, empty output) falls back to Vision for that same capture and records a one-line note in the preview window naming the reason, so a capture always produces text.

**Checks** (all without the model, the runtime or the network): `engineselect` (the engine resolves to the model only with the runtime and correctly sized files present, and to Vision otherwise), `modelmanifest` (two pinned files, 40-character revision, 64-character SHA-256, https URLs, sizes and total matching the spike), `modelverify` (a matching file is accepted, a mismatching one is rejected and deleted), `modelcommand` (the argument vector carries the model path, `--mmproj`, the image, the model card's prompt, `-n 4096` and `--jinja`), and `modelfallback` (a failing launcher raises `nonZeroExit`, a canned launcher's output comes back with the runtime logging and the thinking block stripped). `ovismodel` is the opt-in real run: it is skipped unless `VIZ_OVIS_REAL=1` and never fails the normal suite. The settings tab strip now has five items, so `tabStripMinimumRuns` is 5 (measured share 78 %).

**The recognition preview closes with a glass bubble.** Both preview views (`PreviewContentView` and `ColorPreviewView`) replaced their text `Button("Close")` with the same round control the popover header used: `Button { previewWindow?.orderOut(nil); previewWindow = nil } label: { Image(systemName: "x.circle.fill").font(.system(size: 17)) }` with `.buttonStyle(.plain)`, the theme's `vizGlassBubble()` helper at its default 28 pt and the existing `.help("Close")` tooltip. The close action is byte-identical, the control stays in the top-right corner inside the same `HStack { Spacer(); … }` with the same padding, and nothing else in the file changed - `KeyablePreviewWindow`, `level = .floating`, the transparency, background colour, corner radius, the `viewWidth`/`viewHeight` sizing, the auto-hide timer with its postponement and the post-processing output section are all untouched.

Measured from the preview render's top strip: the close control's widest ink run went from **68 device px** (the word "Close" in a capsule) to **34 px** (the cross-in-a-circle glyph).

In the design-check section a new **`previewclose`** assertion guards it: it renders the preview surface, looks at the top 18 % of the right half, and requires the widest contiguous ink run to stay at or below `previewCloseRunLimit` (45 device px - between the text button's 68 and the bubble's 34) and the control's centre to differ from the panel background by at least `previewCloseSurfaceDeltaMinimum` (30 of 765). Current values: `widestRun=34px surfaceDelta=308`. Honest limit: the surface sample proves a distinct control surface is present, not that it is glass - the harness's material fallback renders glass flat - so the run-width assertion is the real guard.

**The popover buttons are aligned.** SF Symbols have different intrinsic proportions at the same point size - the camera draws about 20 % wider than the viewfinder - so a single shared size made the row look uneven. `RoundedRectangleButtonStyle` now carries a per-symbol point-size table (`viewfinder` 16, `camera` 13, `eyedropper` 15, `clock` 15.5, `delete.left` 14.5, anything else falls back to the passed `size`) and draws every symbol in one 24x18 pt slot, so all five share a vertical centre and the labels below them start at the same y. The icon sizing stays on the font path with `.fixedSize()`; the `.resizable()` path stays out because those images have no intrinsic size and collapsed on macOS 27. The button's vertical padding is asymmetric (top 8, bottom 7): the label's 13 pt line box reserves descender space below its ink, so the block needs one point less underneath to look centred. The slot height is 18 pt rather than 20 pt because 20 pushed the popover's natural height from 99 to 104 pt; it is back at 99 with the same 480 pt width.

Measured (dark render, 2x): icon ink widths went from 29/36/31/30/32 device px (a 7 px spread) to **31/32/31/32/32** (1 px); icon vertical centres 54/55.5/54/53.5/54.5; every label band starts at y=96 and every pill band spans 150-165; the gap above the icon versus below the label is 4.5/3.0, 7.0/5.0, 4.5/5.0, 3.5/2.5 and 5.5/4.5 pt. Icon ink *heights* still differ (31/24/31/32/26 px) - the glyphs have different aspect ratios, so equalising width and height at once is impossible, and width is what reads as size in a horizontal row.

In the design-check section a new **`alignment`** assertion renders the popover at the controller's width and measures each button column, segmenting it by its pill run. It asserts `iconWidthSpread` (limit 4 device px), `iconCentres` (within 3 px of each button centre), `iconRow` (vertical ink centres within 2 px of the row mean), `labelRow` (label tops within 2 px), `inkBalance` (|gap above the icon − gap below the label| within 3 pt, measured inside the button's own fill box) and `pillRow` (pill centres within 2 px). Current values: spread 1 px, worst icon centre 1.3 px, worst icon row 1.2 px, label row 0.0 px, worst ink balance 2.0 pt, worst pill 1.5 px. Two honest limits: the ink-balance metric cannot distinguish the 8/7 padding from a symmetric 9/9 one, because the descender space it compensates for is invisible to ink measurement - the refinement is validated by the render, while the assertion catches real misalignment (a 12/3 padding measures 10.0 pt and fails); and the icon *height* spread is not asserted for the reason above.

**The status button toggles the popover again, and the popover is narrower (supersedes the popover width and toggle notes above).** Clicking the icon while the popover was open did not close it: the click first makes the panel resign key, our resignation path dismissed the panel, and the button action then ran `togglePopover()`, saw `panel.isVisible == false` and reopened it. `dismissPanel()` now records `lastDismissalAt`, a named `toggleSuppressionWindow` (0.3 s) documents that the click that closes the panel is the same click that reaches the button action, and `togglePopover()` refuses to reopen inside that window - a later click opens the popover normally. The click-away monitor also ignores clicks that land on the status item, through the new `statusButtonFrame()` / `isPointOnStatusButton(_:)` helpers (the button-to-screen conversion factored out of `panelFrame(for:)`), so the monitor and the button action cannot fight over the same click. The grace period, the post-grace resignation, Esc, the genuine click-away path and the right-click menu are unchanged.

The popover is **480 pt wide** (it was 600) with the same 99 pt height and the 109 pt ceiling, so every action button is narrower. Two things had to follow: `ContentView` still carried its own `.frame(width: 600)` from the `MenuBarExtra` era, which overrode the panel width and clipped the outer buttons in the render - it now uses `StatusItemController.popoverWidth` - and at 480 pt the five buttons' intrinsic width overflowed the window, so the glass action buttons use 12 pt horizontal padding instead of 16. Every label, symbol and shortcut pill renders fully with no wrapping.

In the design-check section the `statuspanel` lifecycle gained the steps that would have caught the toggle bug and the new width: `secondClickCloses`, `noReopenAfterClickDismissal` (the exact regression - the click's own resignation dismisses the panel and the following button action must not reopen it), `reopensAfterSuppressionWindow` (through the controller's `ignoresToggleSuppression` hook) and `monitorIgnoresStatusButton`, alongside the existing show/monitor/app-resign-active/grace/resign-after-grace/toggle/Esc/click-away steps. The harness also renders the popover at the controller's width constant instead of a hard-coded 600; at the narrower width the icon band is anchored to the label's bright run and measured in bright pixels (the button's own edge crosses every row of the button block, so the backdrop-relative ink profile has no gaps left to split on) and the `popovercontent` transparency check samples the padding corner rather than the dominant colour (at 480 pt the controls cover more than half the surface, so the dominant colour is no longer the backdrop).
