...
/// Turns an image into text. Apple Vision is the app's only recogniser.
...
struct TextRecognition {
    ...
    func recognizeContent() {
        // Exactly as before the engine work: an image with no cgImage is ignored.
        guard image.cgImage(forProposedRect: nil, context: nil, hints: nil) != nil else { return }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            Task {
                do {
                    let combinedText = try await VisionRecognizer().recognize(image: self.image,
